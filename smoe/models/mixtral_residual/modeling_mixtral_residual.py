...
_CONFIG_FOR_DOC = "MixtralResidualConfig"
...
# Copied from transformers.models.llama.modeling_llama.LlamaRMSNorm with Llama->MixtralResidual
class MixtralResidualRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        MixtralResidualRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
# Copied from transformers.models.llama.modeling_llama.LlamaRotaryEmbedding with Llama->MixtralResidual
class MixtralResidualRotaryEmbedding(nn.Module):
...
# Copied from transformers.models.mistral.modeling_mistral.MistralAttention with Mistral->MixtralResidual
class MixtralResidualAttention(nn.Module):
    """
...
class MixtralResidualFlashAttention2(MixtralResidualAttention):
    """
    MixtralResidual flash attention module. This module inherits from `MixtralResidualAttention` as the weights of the module stays
...
class MixtralResidualBLockSparseTop2MLP(nn.Module):
    def __init__(self, config: MixtralResidualConfig, ffn_dim=None):  # 🔍
        super().__init__()
...
MISTRAL_ATTENTION_CLASSES = {
    "eager": MixtralResidualAttention,
...
class MixtralResidualSparseMoeBlock(nn.Module):
    """
    ...
    def __init__(self, config):
        ...
        if self.moe_type == "modulelist":
            self.experts = nn.ModuleList(
                [
                    MixtralResidualBLockSparseTop2MLP(config)
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        final_hidden_states += self.experts_residual(hidden_states)  # 🔍
...
class MixtralResidualDecoderLayer(nn.Module):
    def __init__(self, config: MixtralResidualConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.post_attention_layernorm = MixtralResidualRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
# Copied from transformers.models.mistral.modeling_mistral.MistralPreTrainedModel with Mistral->MixtralResidual
class MixtralResidualPreTrainedModel(PreTrainedModel):
    config_class = MixtralResidualConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["MixtralResidualDecoderLayer"]
...
# Copied from transformers.models.mistral.modeling_mistral.MistralModel with MISTRAL->MIXTRAL,Mistral->MixtralResidual
class MixtralResidualModel(MixtralResidualPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`MixtralResidualDecoderLayer`]

    Args:
        config: MixtralResidualConfig
    """

    def __init__(self, config: MixtralResidualConfig):
        ...
        self.layers = nn.ModuleList(
            [
                MixtralResidualDecoderLayer(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self._use_flash_attention_2 = config._attn_implementation == "flash_attention_2"
        self.norm = MixtralResidualRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        ...
        if attention_mask is not None and self._use_flash_attention_2 and use_cache:
            ...
            if is_padding_right:
                raise ValueError(
                    ...
                    " this may lead to unexpected behaviour for Flash Attention version of MixtralResidual. Make sure to "
...
class MixtralResidualForCausalLM(MixtralResidualPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = MixtralResidualModel(config)
        self.vocab_size = config.vocab_size
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, MixtralResidualModel):
